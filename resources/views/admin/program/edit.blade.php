...
@section('content')
    <form action="{{ route('programs.update', $detail->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="row">
            ...
            <div class="col-xl-8">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <label for="formrow-inputState" class="form-label">Select Level</label>
                            <select id="formrow-inputState" class="form-select @error('level_id') is-invalid @enderror"
                                name="level_id">
                                @foreach ($levels as $value)
                                    <option value="{{ $value->id }}"
                                        {{ $value->id == $detail->level_id ? 'selected' : '' }}>{{ $value->title }}</option>
                                @endforeach
                            </select>
                            @error('level_id')
                                <span class="invalid-feedback" role="alert">
