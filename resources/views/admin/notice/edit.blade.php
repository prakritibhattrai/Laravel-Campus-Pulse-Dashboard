...
@section('content')
    <form action="{{ route('notices.update', $detail->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('put')
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                id="formrow-firstname-input @error('title') is-invalid @enderror" placeholder="Enter Title"
                            ...
                            <select id="formrow-inputState" class="form-select @error('category_id') is-invalid @enderror"
                                ...
                                @foreach ($categories as $value)
                                    <option value="{{ $value->id }}"
                                        {{ $detail->category_id == $value->id ? 'selected' : '' }}>{{ $value->title }}</option>
                                @endforeach
                            </select>
            ...
            <div class="col-xl-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        @if ($detail->image)
                            <div class="mb-3">
                                <img class="img-fluid" src="{{ url($detail->image) }}" height="100" width="100">
                            </div>
                        @endif
