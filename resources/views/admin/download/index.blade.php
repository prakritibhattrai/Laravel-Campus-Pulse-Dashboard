                ...
                <div class="card-body">
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <div class="row">
                            ...
                            <div class="mb-sm-0">
                                <form action="{{ route('downloads') }}" method="get">
                                    <div class="row">
                                        <div class="col-lg-4">
                                ...
                                @if($data->count() > 0)
                                    @foreach ($data as $key => $value)
                                        <tr>
                                            ...
                                            <td>
                                                {{ @$value->resourceCategory->title }}
                                            </td>
                                            <td>
                                                <span class="badge badge-pill badge-soft-{{ $value->status == 'active' ? 'success' : 'danger' }} font-size-11">{{ $value->status}}</span>

                                            </td>

                                            <td>
                                                <form action="{{ route('downloads.destroy', $value->id) }}" method="POST">
                                                    <a href="{{ route('downloads.edit', $value->id) }}"
